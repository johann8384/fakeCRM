...
serverSpecs = {"devtype_group_id": "1",
               "name": "Servers",
               "priority": "1",
               "names": ['implement', 'utilize', 'integrate', 'streamline', 'optimize', 'evolve', 'transform',
                         'embrace', 'enable',
                         'orchestrate', 'leverage', 'reinvent', 'aggregate', 'architect', 'enhance', 'incentivize',
                         'morph',
                         'empower', 'envisioneer', 'monetize', 'harness', 'facilitate', 'seize', 'disintermediate',
                         'synergize',
                         'strategize', 'deploy', 'brand', 'grow', 'target', 'syndicate', 'synthesize', 'deliver',
                         'mesh', 'incubate',
                         'engage', 'maximize', 'benchmark', 'expedite', 'reintermediate', 'whiteboard', 'visualize',
                         'repurpose',
                         'innovate', 'scale', 'unleash', 'drive', 'extend', 'engineer', 'revolutionize', 'generate',
                         'exploit',
...
ticketStatuses = [
  {'id': 0, 'status': 'Open'},
  {'id': 1, 'status': 'Ackknowledged'},
  {'id': 2, 'status': 'Waiting on Vendor'},
...
def generateDevice(typeGroup):
  global deviceid;
  typeGroup = random.choice(typeGroup);
...
def generateTicket():
  ...
  ticketText = "Thank you for being so " \
               + random.choice(adjList).lower() + ". Please " + ticketSubject.lower() + " on my " \
               + random.choice(itemList).lower() + " because I can't use the " + f.catch_phrase().lower()\
               + " while the " + random.choice(itemList).lower() + " is " + random.choice(stateList).lower();
  ...
  if random.choice([True, False]) == True:
    ticketStatus = ticketStatuses[randrange(2, 6)];
    posts = [
      {'adminID': randrange(2, 50), 'name': f.name(), 'subject': f.bs(),
       'body': 'I believe the problem is ' + random.choice(bofhExcuses).lower()
               + '. Please stop trying the ' + f.catch_phrase().lower() + ' on my ' + random.choice(itemList).lower()
      }
    ]
  else:
    ticketStatus = ticketStatuses[randrange(0, 1)];
    posts = []

  ticket = {'ticketID': ticketid, 'author': ticketContact, 'cc': ticketContacts, 'device': ticketDevice,
            'clientID': clientid, 'client': company, 'priority': random.choice(ticketPriorities),
            'status': ticketStatus, 'subject': ticketSubject, 'body': ticketText,
            'priority': random.choice([0, 1, 2, 3]), 'posts': posts
          };
...
for i in range(560):
  ...
  for t in range(randrange(1, 4)):
    ...
    companyDevices.append(
      {'deviceID': device['deviceID'], 'name': device['name'], 'domain': device['domain'], 'ipAddr': device['ipAddr']}
    );
    devices.append(device)

  for t in range(randrange(3, 12)):
    device = generateDevice(deviceTypeAGroups);
    companyDevices.append(
      {'deviceID': device['deviceID'], 'name': device['name'], 'domain': device['domain'], 'ipAddr': device['ipAddr']}
    );
    devices.append(device)
  ...
  if random.choice([True, False]) == True:
    tickets.append(generateTicket());

  client = {'clientID': clientid, 'company': company, 'shortname': shortname, 'catch_phrase': f.catch_phrase(),
            'keywords': f.bs(), 'domain': domain, 'phone': f.phone_number(), 'address1': f.street_address(),
            'address2': f.secondary_address(), 'city': f.city(), 'state': f.state(), 'zip': f.postcode(),
