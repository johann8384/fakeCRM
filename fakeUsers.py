...
for i in range(560):
  ...
  for t in range(randrange(1,4)):
    ...
    deviceid = deviceid + 1;
    device = {'deviceID': deviceid, 'clientID': clientid, 'shortname': shortname, 'domain': domain, 'typeGroupID': typeGroup['devtype_group_id'],  'typeGroupName': typeGroup['name'], 'ipAddr': ipv4(), 'name': shortname + '-' + name + "{0:2d}".format(t+1)}
  ...
  for t in range(randrange(3,12)):
    ...
    deviceid = deviceid + 1;
    ipaddr = ipv4()
    device = {'deviceID': deviceid, 'clientID': clientid, 'shortname': shortname, 'domain': domain, 'typeGroupID': typeGroup['devtype_group_id'],  'typeGroupName': typeGroup['name'], 'ipAddr': ipaddr, 'name': shortname + '-' + name + "{0:2d}".format(t+1)}
  ...
  if random.choice([True, False]) == True:
    for t in range(randrange(0,2)):
      ...
      posts = [
          {'adminID': randrange(2,50), 'name': f.name(), 'subject': f.bs(), 'body': 'I believe the problem is ' + random.choice(bofhExcuses).lower() + '. Please stop trying the ' + f.catch_phrase().lower() + ' on my ' + random.choice(itemList).lower()}
          ]
      ticket = {'ticketID': ticketid, 'author': ticketContact, 'cc': ticketContacts, 'device': ticketDevice, 'clientID': clientid, 'client': company, 'subject': ticketSubject, 'body': ticketText, 'priority': random.choice([0,1,2,3]), 'posts': posts};
